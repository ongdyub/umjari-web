...
import {concertMemberGet, selectConcert} from "../../../store/slices/concert/concert";
            ...
            <Stack sx={{mt: 2, pl:5, pr: 5, mb:10}}>
                {
                    memberState.participants.map((item, idx) => (
                        <Stack key={idx} sx={{mb:2}}>
                            <Stack justifyContent={"flex-start"}>
                                <Typography sx={{fontSize: 35, fontWeight: 100, fontFamily: "Open Sans", mr:5}}>{item.part}</Typography>
                            </Stack>
                            <Stack direction={"row"} alignContent={"center"} justifyContent={"flex-start"} flexWrap={'wrap'} sx={{width : '100%'}}>
                                {item.master.map((item) => (
                                        <Stack key={item.id} direction="row" spacing={5} sx={{mr:2, mb: res500 ? 1 : 0,mt: res500 ? 1 : 0,cursor: 'pointer', height: 48, width: 160}}>
                                ...
                                {item.member.map((item,idx) => (
                                    <Stack key={idx} direction="row" spacing={5} sx={{mr:2, mb:2, cursor: 'pointer', height: 48, width: 130}}>
                                        <Stack sx={{borderRadius: 20, bgcolor: '#eeeeee',pt:1, pl:1, pr:2.5, pb:1}} direction={"row"} alignItems={"center"} alignContent={"center"}>
                                            <Avatar alt={item.profileName} src={`${item.profileImage}`} sx={{width: 33, height: 33}} />
                                            <Typography sx={{ml: 2, fontSize: 15, fontWeight: 400, color:'#424242'}}>{item.profileName}</Typography>
                                        </Stack>
                                    </Stack>
                                ))}
                            </Stack>
